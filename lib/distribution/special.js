...
function betaln(x, y) {
  return _jStat.jStat.gammaln(x) + _jStat.jStat.gammaln(y) - _jStat.jStat.gammaln(x + y);
...
function randn(n, m) {
  var u, v, x, y, q;
  if (!m) m = n;
  if (n) return (0, _core.create)(n, m, function () {
    return randn();
  });

  do {
    u = _jStat.jStat._random_fn();
    v = 1.7156 * (_jStat.jStat._random_fn() - 0.5);
    x = u - 0.449871;
