...
function sub_regress(exog) {
  ...
  var modelList = arange(var_count).map(function (endog_index) {
    ...
    return ols(col(exog, endog_index).map(function (x) {
        return x[0]
      }),
      col(exog, exog_index))
  });
...
function ols(endog, exog) {
  ...
  var coef = lstsq(exog, endog);
  var predict =
    multiply(exog, coef.map(function (x) {
      return [x]
    }))
      .map(function (p) {
        return p[0]
      });
  var resid = subtract(endog, predict);
  var ybar = mean(endog);
  // constant cause problem
  // var SST = jStat.sum(endog.map(function(y) {
  //   return Math.pow(y-ybar,2);
  // }));
  var SSE = sum(predict.map(function (f) {
    return Math.pow(f - ybar, 2);
  }));
  var SSR = sum(endog.map(function (y, i) {
...
export const models = {ols: ols_wrap};
