...
export function pooledvariance(arr) {
...
export function pooledstdev(arr) {
  return Math.sqrt(pooledvariance(arr));
}

// mean deviation (mean absolute deviation) of an array
export function meandev(arr) {
  var meanVal = mean(arr);
  var a = [];
...
export function quantiles(arr, quantilesArray, alphap, betap) {
...
export function percentile(arr, k, exclusive) {
  var _arr = arr.slice().sort(ascNum);
...
export function percentileOfScore(arr, score, kind) {
  var counter = 0;
  var len = arr.length;
  var strict = false;
  var value, i;
...
export function histogram(arr, binCnt) {
  binCnt = binCnt || 4;
  var first = min(arr);
...
export function spearmancoeff(arr1, arr2) {
  arr1 = rank(arr1);
  arr2 = rank(arr2);
...
export function stanMoment(arr, n) {
  var mu = mean(arr);
  var sigma = stdev(arr);
  var len = arr.length;
  var skewSum = 0;
